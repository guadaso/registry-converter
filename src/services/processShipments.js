...
import { extractLast7Digits } from '../utils/data/extractLast7Digits.js';
...
export async function processShipments({ sourceWorkbook, config, files, onLog }) {
    ...
    // ✅ ИСПРАВЛЕНО: totalInputRows = все строки после заголовков
    ...
    const invalidRecords = []; // 🆕 Массив для невалидных записей
    let validModuleCount = 0; // Только строки с хотя бы одним валидным модулем

    const detectedFormat = selectedFormat === 'generic_last7' ? 'generic_last7' : selectedFormat;
...
const rowHasAnyModuleValue = idxModules.some(colIdx => {
        ...
        if (locationInHeader) {
            for (let i = 0; i < idxModules.length; i++) {
                ...
                if (!validation.valid) {
                    ...
                    invalidRecords.push(record); // 🆕 Добавляем в невалидные
                    allRecords.push(record);
                    onLog(`Невалидный модуль "${modVal}" (кв. ${apartment || '—'}, строка ${row + 1}): ${validation.error}`, 'warning');
                    continue;
                }
    ...
    const duplicateRecords = []; // 🆕 Массив для дубликатов

    duplicates.forEach((records, key) => {
        if (records.length > 1) {
            records.forEach(r => {
                r.issue = 'duplicate';
                duplicateRecords.push(r); // 🆕 Добавляем в дубликаты
            });
        } else {
    ...
    const allRecordsWithIssues = [...invalidRecords, ...duplicateRecords]; // 🆕 Все записи с проблемами

    // 🔥 ЛОГИРУЕМ общую статистику до поиска
    ...
    const mainRows = sorted.map(r => {
        const matchStr = foundMap.get(r.searchKey)
            ? `${foundMap.get(r.searchKey).file} — ${foundMap.get(r.searchKey).sheet}`
            : 'Не найден';
        return noLocation
    ...
    const statsRows = [
        ...
        ['Всего номеров модулей в исходном файле', validModuleCount], // ✅ Изменено: теперь это количество модулей, а не строк
    ...
    return {
        ...
        singletonRecords,
        stats: {
            totalInputRows,           // ✅ теперь ВСЕ строки
            removedDuplicates: duplicateRecords.length,
            removedInvalid: invalidRecords.length, // 🆕 Передаем количество невалидных
